...
class Glass:
    def __init__(self, capacity_volume = 200, occupied_volume = 0):
        if not isinstance(capacity_volume, (int, float)):
            raise TypeError("Wrong 'capacity_volume' type of data")
        if capacity_volume < 0:
            raise ValueError("'capacity_volume' value must be more then zero")
        if not isinstance(occupied_volume, (int, float)):
            raise TypeError("Wrong 'capacity_volume' type of data")
        if occupied_volume < 0:
            raise ValueError("'occupied_volume' value must be more then zero")
        if capacity_volume < occupied_volume:
            raise ValueError("'occupied_volume' value must be less then 'capacity_volume'")
...
#
#     def __str__(self):
#         return f"Occupied:  {self.occupied_volume}"
#
#     def __repr__(self):
...
#
#
# g1 = GlassDefaultListArg(200, [100])
# g2 = GlassDefaultListArg(300)
#
# print(g1)
# print(g2)
...
class GlassAddRemove:
    ...
    def remove_water(self, value):
        if value > self.occupied_volume:
            removed = self.occupied_volume
